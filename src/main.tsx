...
if (typeof window !== 'undefined') {
  const topImages = [
    ...NEW_PRODUCTS_CATALOG.slice(0, 16).map(p => p.images?.primary || '').filter(Boolean),
    ...
    ...HD_PRODUCTS.slice(0, 20).map(p => p.images?.primary || (Array.isArray(p.images) ? p.images[0] : '')).filter(Boolean)
  ];
