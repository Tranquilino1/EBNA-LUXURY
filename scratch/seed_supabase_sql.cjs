...
const jsonText = afterMarker.slice(0, endIdx + 1);
const products = eval('(' + jsonText + ')');

console.log(`Extracted ${products.length} products for relational SQL generation.`);
...
products.forEach((p, idx) => {
  ...
  if (!uuidRegex.test(prodId)) {
    const md5 = crypto.createHash('md5').update(`ebna-seed-${p.sku}-${idx}`).digest('hex');
  ...
  const colorsArr = "ARRAY['Blanco','Negro']";
  const sizesArr = "ARRAY['S','M','L']";
...
VALUES ('${prodId}', ${sku}, ${slug}, ${name}, ${brand}, ${category}, ${subcategory}, ${price}, ${priceFcfa}, ${origPrice}, ${inStock}, ${isFeatured}, ${description}, ${imagesJson}::jsonb, ${colorsArr}, ${sizesArr})
...
`;
  ...
  gallery.forEach((gImg, gIdx) => {
    if (gImg !== primaryImg) {
      sql += `INSERT INTO product_images (product_id, image_url, is_primary, display_order) VALUES ('${prodId}', ${escapeSqlStr(gImg)}, FALSE, ${gIdx + 1});\n`;
    }
  });
});

fs.writeFileSync(path.join(__dirname, '../supabase_schema.sql'), sql);
console.log('Successfully generated supabase_schema.sql!');
