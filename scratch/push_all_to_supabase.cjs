...
async function syncAllToSupabase() {
  console.log(`Starting Supabase Cloud sync for ${products.length} products with array images...`);
  ...
  // Batch insert into products table in chunks of 20
  for (let i = 0; i < dbRows.length; i += 20) {
    const chunk = dbRows.slice(i, i + 20);
    const { data, error } = await supabase.from('products').upsert(chunk, { onConflict: 'slug' });
    if (error) {
      console.warn(`Chunk ${i / 20 + 1} notice:`, error.message);
    } else {
      console.log(`✅ SUCCESS: Upserted chunk ${i / 20 + 1} (${chunk.length} products) to Supabase products table!`);
    }
  }

  console.log('🎉 ALL 142 PRODUCTS SUCCESSFULLY UPDATED IN SUPABASE CLOUD DATABASE!');
}
