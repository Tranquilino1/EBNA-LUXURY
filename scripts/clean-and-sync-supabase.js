...
const productsToSync = [
  // 1. CAPTURAS Y SOLICITUDES DEL USUARIO (Precios divididos a la mitad)
  {
  ...
  // 2. PRODUCTOS REALES IMPORTADOS DE FOTOS DEL USUARIO (user-real-1 a user-real-10)
  {
  ...
  {
    id: 'b2222222-2222-4222-8222-222222222208',
  ...
  {
    id: 'b2222222-2222-4222-8222-222222222209',
  ...
  {
    id: 'b2222222-2222-4222-8222-222222222210',
  ...
  // 5. CREMAS PARA PIELES NEGRAS & COSMÉTICA
  {
  ...
  // 7. POMADAS
  {
    id: 'f7777777-7777-4777-8777-777777777701',
  ...
  {
    id: 'f7777777-7777-4777-8777-777777777702',
  ...
  // 8. MODA
  {
    id: 'f8888888-8888-4888-8888-888888888801',
  ...
  {
    id: 'f8888888-8888-4888-8888-888888888802',
  ...
  // 9. PERFUMES & ACCESORIOS DE LUJO
  {
    id: 'f9999999-9999-4999-8999-999999999901',
  ...
  {
    id: 'f9999999-9999-4999-8999-999999999902',
  ...
  {
    id: 'faaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaa01',
...
async function sync() {
  console.log('1. First, fetch existing products from Supabase...');
  ...
  if (existing && existing.length > 0) {
    console.log(`Updating ${existing.length} existing products in Supabase to halved prices...`);
    for (const item of existing) {
      const halvedPrice = Math.max(2000, Math.round(item.price / 2));
      const { error: updateErr } = await supabase
        .from('products')
        .update({ price: halvedPrice, updated_at: new Date().toISOString() })
        .eq('id', item.id);

      if (updateErr) {
        console.error(`Error updating price for ${item.id}:`, updateErr);
      } else {
        console.log(`Updated ${item.name} (${item.id}): ${item.price} -> ${halvedPrice} FCFA`);
      }
    }
  }

  console.log('\n2. Upserting target user products with valid UUIDs...');
  for (const item of productsToSync) {
    ...
    if (error) {
      console.error(`Error upserting ${item.id} (${item.name}):`, error);
    } else {
      console.log(`Upserted ${item.name} (${item.price} FCFA)`);
    }
  }

  const { data: final } = await supabase.from('products').select('id, name, price, images');
  console.log(`\nDONE! Total products in Supabase now: ${final ? final.length : 0}`);
}
